...
sweep_config = {
    ...
    "parameters": {
        ...
        "epochs": {"value": 140},
        "optimizer": {"value": "sgd"},
        "scheduler": {"value":"step_lr"},
        "dataset":{"value":"obs_loc10_rot90"},
        "augmentation":{"values":["invert"]},
    },
}
sweep_id = wandb.sweep(sweep = sweep_config, project = "NN_localization_sweep3")
# sweep_id = "blast1211/NN_localization_sweep3/oyinom5z"

def visualizer(dataset,model, train_data):
    for data in dataset:
        map_img, crop_img, loc,rot, loc_conf,rot_conf = data["map_img"], data["crop_img"], data["location"], data["rotation"], data["loc_conf"],data["rot_conf"]
        out_loc, out_rot,out_loc_conf,out_rot_conf = model(map_img.cuda(), crop_img.cuda())
        out_loc,out_rot,out_loc_conf, out_rot_conf = out_loc.detach().cpu().numpy(), out_rot.detach().cpu().numpy(), out_loc_conf.detach().cpu().numpy(), out_rot_conf.detach().cpu().numpy()
        ...
        for num in range(len(out_loc)):
            ...
            ax[0][num].set_title(f"l{np.round(out_loc_conf[num],3)}_r{np.round(out_rot_conf[num],3)}")
            ...
            ax[1][num].set_title(f"l{np.round(loc_conf[num],3)}_r{np.round(rot_conf[num],3)}")

        break
    return fig

def compute_loss(data,model,loc_crit,rot_crit, loc_conf_crit, rot_conf_crit):
    map_img, crop_img, loc,rot, loc_conf, rot_conf = data["map_img"], data["crop_img"], data["location"], data["rotation"], data["loc_conf"],data["rot_conf"]
    loc, rot, loc_conf, rot_conf = loc.float().cuda(), rot.float().cuda(), loc_conf.float().cuda(), rot_conf.float().cuda()
    out_loc, out_rot, out_loc_conf, out_rot_conf = model(map_img.cuda(), crop_img.cuda())

    loc_loss = torch.sum(loc_crit(out_loc, loc),dim=1)
    rot_loss = rot_crit(out_rot, rot)
    loc_conf_loss = loc_conf_crit(out_loc_conf, loc_conf[:,None])
    rot_conf_loss = rot_conf_crit(out_rot_conf, rot_conf[:,None])

    loss =  (torch.matmul(loc_loss,(out_loc_conf)) - 2*torch.mean(torch.log(out_loc_conf)))/len(out_loc_conf)\
          + (torch.matmul(rot_loss,(out_rot_conf)) - 2*torch.mean(torch.log(out_rot_conf)))/len(out_rot_conf) \
          + 2*loc_conf_loss + 2*rot_conf_loss
    return loss, torch.mean(loc_loss),torch.mean(rot_loss), loc_conf_loss, rot_conf_loss
...
def main():
    ...
    model = models.NN_Localization_Conf(rot_out_num=dataset_dict[wandb.config.dataset]["class_num"])
    model.load_state_dict(torch.load("../weights/GSC_C_invert_obs_loc10_rot90_loss_invert_76ep.pt")["model_state_dict"])
    model = model.cuda()

    optimizer_dict = {"adam":torch.optim.Adam(model.parameters(), lr=1e-3),
                      "sgd":torch.optim.SGD(model.parameters(), lr=4e-4, momentum=0.9)}
    optimizer = optimizer_dict[wandb.config.optimizer]


    scheduler_dict = {"step_lr":torch.optim.lr_scheduler.StepLR(optimizer, step_size=2, gamma=0.9),
                      "chained1":ChainedScheduler(optimizer, T_0 = 20, T_mul = 2,eta_min = 0.0,gamma = 0.7,max_lr = 0.001,warmup_steps= 3 ,),
                      "chained2":ChainedScheduler(optimizer, T_0 = 5, T_mul = 2,eta_min = 0.0,gamma = 0.5,max_lr = 0.001,warmup_steps= 2 ,),
                      }
    ...
    loc_criterion = torch.nn.L1Loss(reduction="none")
    rot_criterion = torch.nn.CrossEntropyLoss(reduction="none")
    loc_conf_criterion = torch.nn.BCELoss()
    rot_conf_criterion = torch.nn.BCELoss()
    ...
    train_loader = torch.utils.data.DataLoader(dataset = train_data, batch_size=batch_size, num_workers=4,shuffle = True, drop_last=True, persistent_workers=True)
    test_loader  = torch.utils.data.DataLoader(dataset = test_data,  batch_size=10, num_workers=4,shuffle = False, drop_last=True, persistent_workers=True)
    valid_loader = torch.utils.data.DataLoader(dataset = valid_data, batch_size=10, num_workers=4,shuffle = False, drop_last=True, persistent_workers=True)

    loss_batch_size = len(train_loader)
    

    model_name = f"GSC_C_{wandb.config.augmentation}_{wandb.config.dataset}_resume"
    ...
    for ep in range(epoch):
        ...
        loc_loss_total = 0.0
        rot_loss_total = 0.0
        loc_conf_loss_total = 0.0
        rot_conf_loss_total = 0.0
        loss_it = 0
        model.train()
        ...
        for i, data in enumerate(train_tqdm):
            ...
            loss_it +=1
            optimizer.zero_grad()
            loss, loc_loss, rot_loss, loc_conf_loss, rot_conf_loss = compute_loss(data,model,loc_criterion,rot_criterion, loc_conf_criterion, rot_conf_criterion)
            loss.backward() 
            optimizer.step()
            ...
            loc_conf_loss_total += loc_conf_loss.item()
            rot_conf_loss_total += rot_conf_loss.item()
            ...
            if it%log_per_it==0:
                wandb.log({
                    ...
                    "loc_conf_loss": loc_conf_loss_total/loss_it,
                    "rot_conf_loss": rot_conf_loss_total/loss_it,
        ...
        for i ,data in valid_tqdm:
            loss, _,_,_,_ = compute_loss(data,model,loc_criterion,rot_criterion, loc_conf_criterion, rot_conf_criterion)
        ...
        for i ,data in enumerate(tqdm(test_loader)):
            ...
            out_loc,out_rot,_,_ = model(map_img.cuda(), crop_img.cuda())
