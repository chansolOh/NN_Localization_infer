...
sweep_config = {
    ...
    "parameters": {
        ...
        "dataset":{"value":"newmap_loc20_rot90"},
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(dataset = train_data, batch_size=batch_size, num_workers=6,shuffle = True, drop_last=True, persistent_workers=True)
    test_loader  = torch.utils.data.DataLoader(dataset = test_data,  batch_size=10, num_workers=6,shuffle = False, drop_last=True, persistent_workers=True)
    valid_loader = torch.utils.data.DataLoader(dataset = valid_data, batch_size=10, num_workers=6,shuffle = False, drop_last=True, persistent_workers=True)

    loss_batch_size = len(train_loader)
    

    model_name = f"GSC_C_{wandb.config.augmentation}_{wandb.config.dataset}_loss_invert"
